...
export default function Tours() {
  ...
  const toSort = (tours: TourType[], id: sortIdType) => {
    switch (id) {
      case 'popular':
  ...
  return (
    ...
    <>
      <NextSeo
        ...
        canonical="http://localhost:3000/tours"
        openGraph={{
                  ...
                  <FormGroup>
                    {
                      activities.map(item => 
                        <FormControlLabel key={item?.id} control={
                          <Checkbox
                            onChange={(e) => {
                              if(e.currentTarget.checked) dispatch(fetchTourFilterActivitiesAdd(item.title))
                              else dispatch(fetchTourFilterActivitiesRemove(item.title))
                            }}
                          />
                        } 
                        label={item?.title} />
